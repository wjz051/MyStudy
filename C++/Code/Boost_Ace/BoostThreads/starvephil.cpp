...
public:
    ...
    void put(int value) {
        ...
        {
            boost::mutex::scoped_lock lock(iomx);
            std::cout << "(" << clock() << ") Chef: more chickens ... "
                    << m_chickens << " now available ... NOTIFYING ..."
                    << std::endl;
        }
...
int main(int argc, char* argv[]) {
    ...
    boost::thread thrd_phil0(&phil::run, &p[0]);
    boost::thread thrd_phil1(&phil::run, &p[1]);
    boost::thread thrd_phil2(&phil::run, &p[2]);
    boost::thread thrd_phil3(&phil::run, &p[3]);
    boost::thread thrd_phil4(&phil::run, &p[4]);
